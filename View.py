...
class View(Tk):
    def __init__(self, controller, model):
        ...
        self.__frame_bottom, self.__frame_top = self.create_frames()
    ...
    def create_buttons(self):
        ...
        open_btn = Button(self.__frame_top, text='Ava', font=self.__default, command=self.__controller.open_file,
                          bg="#C70039", fg="#ffffff", width=12, height=3)
    ...
    def create_labels(self):
        ...
        self.__file_name_lbl = Label(
            self.__frame_top,
            text='Vali esmalt fail!',
            anchor='w',
            font=self.__default,
            fg='red'
        )
        self.__file_name_lbl.grid(row=1, column=1, padx=5, pady=2, sticky=EW)

        self.__row_count_lbl = Label(
            self.__frame_top,
            text='Ridu kokku:',
            anchor='w',
            font=self.__default
        )
        self.__row_count_lbl.grid(row=1, column=2, padx=5, pady=2, sticky=EW)
        self.__row_count_lbl.grid_remove()

        self.__count_lbl = Label(
            self.__frame_top,
            text='0',
            anchor='w',
            font=self.__default
        )
        self.__count_lbl.grid(row=1, column=3, padx=5, pady=2, sticky=EW)
        self.__count_lbl.grid_remove()
        ...
        return search_lbl, open_file_lbl, self.__file_name_lbl, self.__row_count_lbl, self.__count_lbl, error_lbl
